...
@Composable
fun TextPackSheet(
    textFiles: List<PutioFile>,
    onDismiss: () -> Unit,
    onConfirm: (selectedFiles: List<PutioFile>) -> Unit,
    readStubFileSize: suspend (PutioFile) -> Long?,
) {
    var caseSensitiveSort by remember(textFiles) { mutableStateOf(false) }
    var orderedFiles by remember(textFiles, caseSensitiveSort) {
        mutableStateOf(MetadataUtils.sortByName(textFiles, caseSensitiveSort) { it.displayName })
    }
    var checkedIds by remember(textFiles) { mutableStateOf(textFiles.map { it.id }.toSet()) }
    val selectedFiles = orderedFiles.filter { it.id in checkedIds }
    var collapseNames by remember { mutableStateOf(true) }
    val sizeProgress = rememberSizeProgress(textFiles, readStubFileSize)
    ...
    Dialog(
        ...
        Surface(
            ...
            Column(
                ...
                Text(
                    text = "Select files to join into PDF",
                ...
                Text(
                    text = "Files are converted to PDF and merged in the listed order — use ▲▼ to reorder.",
                ...
                Row(
                    ...
                    Text(
                        text = "${selectedFiles.size} of ${textFiles.size} files selected" + sizeProgressSuffix(sizeProgress, selectedFiles),
                    ...
                    TextButton(
                        onClick = {
                            checkedIds = if (checkedIds.size == textFiles.size) {
                                emptySet()
                            } else {
                                textFiles.map { it.id }.toSet()
                            }
                        },
                    ) {
                        Text(if (checkedIds.size == textFiles.size) "Deselect all" else "Select all")
                    }
                }
