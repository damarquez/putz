...
private val MIGRATION_26_27 = object : Migration(26, 27) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL(
            "CREATE TABLE IF NOT EXISTS `pending_response_deletions` (`fileId` TEXT NOT NULL, `discoveredAt` INTEGER NOT NULL, PRIMARY KEY(`fileId`))"
        )
    }
}
...
private val MIGRATION_27_28 = object : Migration(27, 28) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN chainId TEXT")
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN chainPosition INTEGER")
    }
}

private val MIGRATION_28_29 = object : Migration(28, 29) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN pendingCoverPutioFileId INTEGER")
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN pendingCoverDownloadUrl TEXT")
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN pendingCoverFileName TEXT")
    }
}

private val MIGRATION_29_30 = object : Migration(29, 30) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN ignoreCover INTEGER NOT NULL DEFAULT 0")
    }
}

private val MIGRATION_30_31 = object : Migration(30, 31) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN comments TEXT")
    }
}
...
private val MIGRATION_31_32 = object : Migration(31, 32) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL(
            "CREATE TABLE IF NOT EXISTS `folder_display_names` (`putioFileId` INTEGER PRIMARY KEY NOT NULL, `displayName` TEXT NOT NULL, `updatedAt` INTEGER NOT NULL)"
        )
    }
}
...
private val MIGRATION_32_33 = object : Migration(32, 33) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN pendingCoverUseLocal INTEGER NOT NULL DEFAULT 0")
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN pendingCoverLocalPath TEXT")
    }
}

private val MIGRATION_33_34 = object : Migration(33, 34) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("ALTER TABLE calibre_transfers ADD COLUMN parentFolderId INTEGER")
    }
}
...
object DatabaseModule {
    ...
    @Provides
    fun providePendingResponseDeletionDao(db: PutzDatabase) = db.pendingResponseDeletionDao()

    @Provides
    fun provideFolderDisplayNameDao(db: PutzDatabase) = db.folderDisplayNameDao()
}
